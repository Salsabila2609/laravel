  ...
  <div>
    <Background />
      <div class="text-sm text-[#99cbc0] font-bold p-5">
        <Link href="/dashboard" class="text-yellow-500 no-underline">Beranda</Link> > Local Government Organization
      </div>

    <div class="content relative z-10 p-4 sm:p-5 text-gray-800">
      <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-[#D4A017] mb-4 sm:mb-6">LOCAL GOVERNMENT ORGANIZATION</h1>
      <AddButton 
        @click="openAddModal" 
        buttonText="Add New OPD" 
      ...
      <Card class="mt-4 sm:mt-6">
        <div class="overflow-x-auto">
          <table class="w-full mx-auto table-auto border-collapse">
            <thead>
              <tr>
                <th class="py-2 px-2 sm:px-4 border-b text-center text-sm sm:text-base min-w-[150px]">Nama OPD</th>
            ...
            <tbody>
              <tr v-for="opd in opds" :key="opd.id">
                <td class="py-2 px-2 sm:px-4 border-t border-b text-sm sm:text-base">
                  <a :href="opd.link || '#'" class="text-blue-500 hover:underline break-words" target="_blank">
                    {{ opd.name }}
                  </a>
                </td>
                <td class="py-2 px-2 sm:px-4 border-t border-b text-sm sm:text-base">{{ opd.address }}</td>
                <td class="py-2 px-2 sm:px-4 border-t border-b text-sm sm:text-base">{{ opd.contact }}</td>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <div class="bg-white p-4 sm:p-6 rounded-lg shadow-lg w-11/12 sm:w-full max-w-md">
          <h2 class="text-center text-xl sm:text-2xl font-bold text-yellow-500 mb-4">
            {{ isEditMode ? 'Edit OPD' : 'Add New OPD' }}
          </h2>
          <form @submit.prevent="handleSubmit">
            <div class="mb-4">
              <label for="name" class="block text-sm font-medium text-gray-700">Nama OPD</label>
              <input v-model="form.name" type="text" id="name" class="w-full mt-2 p-2 border rounded-md text-sm" placeholder="Nama OPD" required />
            </div>
            <div class="mb-4">
              <label for="address" class="block text-sm font-medium text-gray-700">Alamat</label>
              <input v-model="form.address" type="text" id="address" class="w-full mt-2 p-2 border rounded-md text-sm" placeholder="Alamat OPD" required />
            </div>
            <div class="mb-4">
              <label for="contact" class="block text-sm font-medium text-gray-700">Kontak</label>
              <input v-model="form.contact" type="text" id="contact" class="w-full mt-2 p-2 border rounded-md text-sm" placeholder="Kontak OPD" required />
            </div>
            <div class="mb-4">
              <label for="link" class="block text-sm font-medium text-gray-700">Link</label>
              <input v-model="form.link" type="text" id="link" class="w-full mt-2 p-2 border rounded-md text-sm" placeholder="Link OPD" />
            </div>
